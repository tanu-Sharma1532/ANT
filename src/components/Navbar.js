      ...
      <div className="max-w-[1400px] mx-auto flex items-center justify-between px-8"> {/* reduced py */}
        ...
        <div className="flex items-center gap-12">
          ...
          <div className="flex items-center text-sm font-medium gap-2">
            {["Home", "Bus Ticket", "Bus Hire", "Car Hire", "Traveller Hire", "Gallery", "Online Payment", "Send Enquiry", "Contact Us"].map((item, idx) => (
              <div key={idx} className="flex items-center">
                <a className="px-2 py-0.5 hover:text-[#ff9900] transition-all duration-300 font-medium">
                  {item}
                </a>
                {idx !== 8 && <span className="text-gray-400">|</span>} 
              </div>
            ))}
          </div>
        </div>
